...
const postAdditionalDetails = async (payload) => {
    ...
    saveToLS('auth-details-request', formatJSON(payload));
...
const handleOnSubmit = async (state, _passedComponent) => {
    ...
    postRequest('/makePayment', data)
        .then(response => {
            ...
            saveToLS('auth-request', formatJSON(response.request));
...
const additionalDetailsComplete = (response) => {
    ...
    console.log(`pspreference ${pspReference}`);
    if (type === 'dropin') {
        try {
            component.setStatus('success', { message: 'Reload to make another payment request.' });
        } catch (e) {
    ...
    saveToLS("auth-pspReference", pspReference);

    // Update status on UI 
    updateStatus(resultCode);
    saveToLS('auth-resultCode', resultCode);

    // Updates the request time on the UI 
    const date = updateRequestTime();
    saveToLS('auth-requestTime', date);
};
...
const paymentComplete = (response) => {
    ...
    saveToLS('auth-response', formatJSON(response));
    ...
    if (resultCode === undefined) {
        updateStatus('Error');
        saveToLS('auth-resultCode', 'Error');
    } else {
        updateStatus(resultCode);
        saveToLS('auth-resultCode', resultCode);
    }
...
const getItemsFromLS = () => {
    ...
    const pspReference = getFromLS('auth-pspReference');
    updatePspReference(pspReference);

    // Get result code and render 
    const resultCode = getFromLS('auth-resultCode');
    updateStatus(resultCode);

    // Get checkout request and render
    try {
        const paymentRequest = JSON.parse(getFromLS('auth-request'));
    ...
    try {
        const paymentResponse = JSON.parse(getFromLS('auth-response'));
    ...
    let requestTime = getFromLS('auth-requestTime');
    if (!requestTime) requestTime = "";
    updateRequestTime(requestTime);

    // Get additional details call and render 
    const detailsRequest = JSON.parse(getFromLS('auth-details-request'));
